...
export default function MobileCartPage() {
  ...
  return (
    <AppPageLayout hasBottomNav={true}>
      <AppHeader variant="contextual" title="Bag" fallbackUrl="/mobile" />
      ...
      <div className="flex flex-col w-full bg-zinc-50 dark:bg-zinc-950 min-h-full pb-32">
        ...
        <div className="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 px-4 py-3">
          <DeliveryLocationSheet triggerClassName="flex items-center gap-1.5 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors px-3 py-2 rounded-full text-[11px] font-medium text-zinc-700 dark:text-zinc-200 focus:outline-none" />
        </div>

        {cart.length === 0 ? (
          // ─── EMPTY STATE ───
          <div className="flex-1 flex flex-col items-center justify-center pt-12 pb-6 bg-zinc-50 dark:bg-zinc-950">
            <div className="w-24 h-24 bg-zinc-200/50 dark:bg-zinc-800 rounded-3xl flex items-center justify-center mb-6 shadow-sm border border-zinc-200 dark:border-zinc-700">
              <ShoppingBag className="w-10 h-10 text-[#6F4E37] dark:text-[#E6C280] stroke-2" />
            </div>
            ...
            <Link 
              href="/mobile/shop"
              className="flex items-center gap-2 bg-zinc-950 dark:bg-white text-white dark:text-zinc-950 px-8 py-3.5 rounded-xl font-mono text-xs font-bold uppercase tracking-wider active:scale-95 transition-transform shadow-lg"
            >
              Explore Drops
            ...
            {wishlist.length > 0 && (
              <div className="w-full mt-16 text-left bg-white dark:bg-zinc-900 pt-8 pb-10 border-t border-zinc-200 dark:border-zinc-800">
                <div className="mb-6 px-4 flex flex-col gap-1 text-center">
                  <span className="text-[10px] font-mono font-bold tracking-[0.25em] text-[#6F4E37] dark:text-[#E6C280] uppercase">
                    YOUR CURATION
                  </span>
                  <h3 className="text-xl font-light tracking-tight text-zinc-900 dark:text-white font-playfair uppercase">
                    From Your <span className="font-serif italic font-normal text-[#6F4E37] dark:text-[#E6C280]">Wishlist</span>
                  </h3>
                </div>
                <div className="flex overflow-x-auto gap-4 pb-6 snap-x snap-mandatory scrollbar-none px-4 pl-4 pr-6">
                  {wishlist.map((item) => {
                    ...
                    return (
                      <div key={item.id} className="w-[160px] shrink-0 snap-start">
                        <ProductCard
                          id={item.id}
                          name={item.name}
                ...
                <div key={`${item.id}-${item.size}-${item.color}`} className="p-4 flex flex-col gap-4">
                  ...
                  <div className="flex items-center justify-between border-t border-zinc-100 dark:border-zinc-800 pt-3">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      >
                        <Heart 
                          className={`w-4 h-4 ${isInWishlist(item.id) ? "text-red-500" : ""}`} 
