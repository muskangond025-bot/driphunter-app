...
export default function HelpCenterPage() {
  ...
  return (
    <AppPageLayout hasBottomNav={false} className="bg-background text-foreground flex flex-col font-sans transition-colors duration-300">
      <AppHeader 
        variant="contextual" 
        title="24 x 7 Customer Support" 
        fallbackUrl="/mobile/account" 
        onBack={isMobileDetailView ? () => setIsMobileDetailView(false) : undefined}
      />
      ...
      <main className="flex-grow pt-8 pb-16 px-4 md:px-8 max-w-[1400px] mx-auto w-full relative">
        ...
        <div className="flex flex-col md:flex-row bg-transparent gap-4 mb-8">
          {/* Left Sidebar */}
          <div className={`w-full md:w-[280px] flex-shrink-0 bg-white dark:bg-zinc-900/60 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm ${isMobileDetailView ? 'hidden md:flex flex-col' : 'flex flex-col'}`}>
                ...
                {issues.map(issue => (
                  <div 
                    ...
                    onClick={() => {
                      ...
                      if (issue !== "Help with other issues") {
          ...
          {/* Right Main Content */}
          <div className={`flex-1 bg-transparent gap-4 ${isMobileDetailView ? 'flex flex-col' : 'hidden md:flex flex-col'}`}>
