...
import { ArrowLeft, RotateCcw, Camera, X, CheckCircle2 } from "lucide-react";
...
export default function ReturnOrderPage() {
  ...
  const handleReturn = async (e: React.FormEvent) => {
    ...
    if (returnReason === "Other" && !otherReason.trim()) return;
    ...
    const returnedOrders = JSON.parse(localStorage.getItem("drip_returned_orders") || "[]");
    if (!returnedOrders.includes(orderId)) {
      returnedOrders.push(orderId);
      localStorage.setItem("drip_returned_orders", JSON.stringify(returnedOrders));
    }
    ...
    <div className="flex flex-col min-h-screen bg-[#faf8f5] dark:bg-[#0a0a0c] text-zinc-900 dark:text-white font-sans antialiased">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tight font-sans flex items-center gap-4">
              <RotateCcw className="w-10 h-10 text-zinc-900 dark:text-white" />
              Return / Exchange
            </h1>
            <p className="text-zinc-500 dark:text-zinc-400 text-sm font-mono max-w-xl">
              You are requesting to return or exchange order <strong className="text-zinc-900 dark:text-white">#{orderId}</strong>.
            </p>
          </div>
        </div>

        <form onSubmit={handleReturn} className="bg-white dark:bg-zinc-900/80 rounded-3xl p-8 sm:p-12 shadow-sm border border-zinc-200 dark:border-zinc-800 space-y-8">
          <div>
            <h3 className="text-xl font-bold font-sans text-zinc-900 dark:text-white mb-6">Why are you returning this?</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {["Size doesn't fit", "Damaged product", "Received wrong item", "Quality not as expected", "Changed my mind", "Other"].map((reason) => (
                <label 
                  key={reason} 
                ...
                >
                  <div className={`mt-0.5 w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${
                    returnReason === reason ? 'border-[#6F4E37] dark:border-[#E6C280]' : 'border-zinc-300 dark:border-zinc-700'
                  }`}>
                    {returnReason === reason && <div className="w-2.5 h-2.5 rounded-full bg-[#6F4E37] dark:bg-[#E6C280]" />}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    {attachedImage && (
                      <div className="relative w-20 h-20 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden group">
                        <img src={attachedImage} alt="Attached photo" className="w-full h-full object-cover" />
          ...
          <div className="pt-8 border-t border-zinc-200 dark:border-zinc-800 flex flex-wrap gap-4 items-center justify-end">
            ...
            <button
              type="submit"
              disabled={!returnReason || (returnReason === "Other" && !otherReason.trim()) || isSubmitting}
              className="px-8 py-4 rounded-full text-xs font-mono font-bold uppercase tracking-widest bg-zinc-900 dark:bg-white hover:bg-zinc-800 dark:hover:bg-zinc-100 text-white dark:text-zinc-900 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer border-none shadow-md"
            >
      ...
      <Dialog open={showSuccessModal} onOpenChange={setShowSuccessModal}>
        <DialogContent
          ...
          className="bg-white dark:bg-zinc-900 rounded-[32px] w-full max-w-sm p-8 text-center shadow-2xl border border-zinc-200 dark:border-zinc-800 gap-0"
        >
          ...
          <DialogTitle className="text-2xl font-black uppercase tracking-tight font-sans text-zinc-900 dark:text-white mb-2 leading-none">
            Return Initiated
          </DialogTitle>
          <DialogDescription className="text-sm font-mono text-zinc-500 dark:text-zinc-400 mb-8 text-center">
            Your return request for order #{orderId} has been successfully submitted. You will receive an email shortly with return instructions.
          </DialogDescription>
          <button
