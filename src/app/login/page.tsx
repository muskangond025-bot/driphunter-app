...
export default function LoginPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulate user login
    localStorage.setItem("drip_user", JSON.stringify({ name: "Demo User", email }));
  ...
  return (
    <div className="flex flex-col min-h-screen bg-zinc-50 dark:bg-[#0a0a0a] text-foreground">
      ...
      <main className="flex-grow relative flex items-center justify-center overflow-hidden select-none py-20 px-4">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden z-0 flex items-center justify-center">
          ...
          {/* Ambient Glow Orbs */}
          <div className="absolute top-1/4 left-1/4 w-[40vw] h-[40vw] bg-[#6F4E37]/10 dark:bg-[#6F4E37]/20 rounded-full blur-[100px] animate-pulse"></div>
          <div className="absolute bottom-1/4 right-1/4 w-[50vw] h-[50vw] bg-zinc-400/10 dark:bg-zinc-800/40 rounded-full blur-[120px]"></div>
        </div>

        {/* ─── GLASSMORPHISM FORM CONTAINER ─── */}
        <div className="relative z-10 w-full max-w-[480px] animate-fade-in mt-12 sm:mt-0">
          ...
          <div className="bg-white/70 dark:bg-black/40 backdrop-blur-2xl border border-zinc-200/50 dark:border-white/10 rounded-[2.5rem] p-8 sm:p-12 shadow-[0_0_80px_rgba(0,0,0,0.05)] dark:shadow-[0_0_80px_rgba(0,0,0,0.4)] relative">
            ...
            <button 
              onClick={() => router.push("/")}
              className="absolute top-6 right-6 p-2 rounded-full text-zinc-400 hover:text-zinc-900 dark:hover:text-white transition-all"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="space-y-2 mb-12 text-center">
              ...
              <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-zinc-900 dark:text-white font-sans uppercase">
                Access <span className="font-serif italic font-normal text-[#6F4E37] dark:text-[#E6C280]">Vault</span>
              </h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="flex justify-between items-center pt-2">
                <div className="flex items-center gap-1.5 opacity-60">
                  ...
                  <span className="text-[9px] text-zinc-900 dark:text-white font-mono uppercase tracking-widest font-bold">256-bit SSL</span>
                </div>
                <Link
                  href="/forgot-password"
                  className="text-[9px] font-mono text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white font-bold uppercase tracking-widest transition-colors"
                >
                  Forgot Password?
                </Link>
              </div>
