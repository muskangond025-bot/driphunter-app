...
with open("src/app/mobile/product/[id]/page.tsx", "r", encoding="utf-8") as f:
    content = f.read()
...
if 'import Footer from "@/components/layout/Footer";' not in content:
...
# 2. Add Breadcrumbs after AppHeader
breadcrumbs_code = """        }
      />
...
content = re.sub(r'        \}\n      />', breadcrumbs_code, content, count=1)


# 3. Rest of the sections replacement
# We need to replace everything from `        <div className="h-2 w-full bg-zinc-50 dark:bg-zinc-900" />\n\n        {/* 9. REVIEWS */}` 
# down to the end of `SIMILAR PRODUCTS` section. Wait, let's just replace from `{/* 9. REVIEWS */}` down to before `{/* ─── TOAST NOTIFICATION ─── */}`

start_marker = "{/* 9. REVIEWS */}"
...
if idx_start != -1 and idx_end != -1:
    new_sections = """{/* 6.5 VIRTUAL FITTING ROOM */}
    ...
    content = content[:idx_start] + new_sections + content[idx_end:]

with open("src/app/mobile/product/[id]/page.tsx", "w", encoding="utf-8") as f:
    f.write(content)

print("Updated mobile product page!")
